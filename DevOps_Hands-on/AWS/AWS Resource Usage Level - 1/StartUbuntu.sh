...
key_pair_name="UbuntuKeyPair"
if ! aws ec2 describe-key-pairs --key-names ${key_pair_name} --region ${region} &>/dev/null; then
    aws ec2 create-key-pair --key-name ${key_pair_name} --query 'KeyMaterial' --output text --region ${region} > UbuntuKeyPair.pem
    # Set permissions for the key pair file
    chmod 400 UbuntuKeyPair.pem
else
...
security_group_name="UbuntuSecurityGroup"
...
if [ "$security_group_id" == "None" ]; then
    security_group_id=$(aws ec2 create-security-group --group-name ${security_group_name} --description "Ubuntu security group" --vpc-id ${vpc_id} --query 'GroupId' --output text --region ${region})
...
instance_id=$(aws ec2 run-instances \
    ...
    --user-data file://userDataUbuntu.sh \
    --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value='${instance_name}'}]' \
    --region ${region} \
    --query 'Instances[0].InstanceId' --output text)

# Store resource IDs
echo "KeyPairName=${key_pair_name}" > resource_ids_ubuntu.txt
echo "VpcId=${vpc_id}" >> resource_ids_ubuntu.txt
echo "SubnetId=${subnet_id}" >> resource_ids_ubuntu.txt
echo "SecurityGroupId=${security_group_id}" >> resource_ids_ubuntu.txt
echo "InstanceId=${instance_id}" >> resource_ids_ubuntu.txt
echo "IgwId=${igw_id}" >> resource_ids_ubuntu.txt
echo "RouteTableId=${route_table_id}" >> resource_ids_ubuntu.txt
